...
public sealed class WindowsPlatform : IPlatform
{
    ...
    private bool ShouldResleepOnWakeReason(ModernStandbyResleepMonitor.WakeReason reason)
    {
        return reason != ModernStandbyResleepMonitor.WakeReason.PowerButton
            && reason != ModernStandbyResleepMonitor.WakeReason.Fingerprint;
    }
    ...
    private sealed class ModernStandbyResleepMonitor
    {
        ...
        // Grace window: after a legitimate unlock (PowerButton/Fingerprint), ignore other wake events briefly
        ...
        private void DisposeWatcher()
        {
            ...
            {
                w.Enabled = false;
                w.EventRecordWritten -= OnEventRecordWritten;
            }
            catch { }

            // Dispose on background thread to prevent deadlock:
            // UI thread may be waiting in Dispose() while callback's SendMessage
            // needs the UI thread to process the broadcast message.
            Task.Run(() => { try { w.Dispose(); } catch { } });
        }
        ...
        private void OnPowerModeChanged(object sender, PowerModeChangedEventArgs e)
        {
            ...
            lock (_lock)
            {
                ...
                LogManager.LogInformation("[GoBackToSleep] System resumed, restarting event watcher...");
                ...
                {
                    LogManager.LogError("[GoBackToSleep] Failed to restart watcher after resume: {0}", ex.Message);
                }
            }
        }
        ...
        private void OnEventRecordWritten(object sender, EventRecordWrittenEventArgs e)
        {
            ...
            if (eventId == 507) // wake
            {
                ...
                {
                    // Legitimate unlock — set a 1-second grace window to absorb companion events
                    Interlocked.Exchange(ref _graceUntilTicks, now + TimeSpan.FromSeconds(2).Ticks);
                    LogManager.LogInformation("[GoBackToSleep] Wake reason {0} is excluded, NOT resleeping. Grace window set.", reason);
                    return;
                }

                // Inside grace window after a legitimate unlock — ignore companion events
                if (now < Interlocked.Read(ref _graceUntilTicks))
                {
                    LogManager.LogInformation("[GoBackToSleep] Wake reason {0} ignored (within grace window of legitimate unlock).", reason);
                    return;
                }
        ...
        {
            // Fire on background thread to avoid blocking the event callback.
            // SendMessage(HWND_BROADCAST, ...) is synchronous and can deadlock
            // if the UI thread is blocked in _watcher.Dispose().
            Task.Run(() =>
            {
                try { SendMessage(HWND_BROADCAST, WM_SYSCOMMAND, SC_SUSPEND, 2); }
                catch { }
            });
        }
        ...
        private WakeReason ParseWakeReason(EventRecord evt)
        {
            try
            {
                ...
                {
                    LogManager.LogWarning("[GoBackToSleep] Failed to parse Reason value: '{0}'", reasonVal);
                ...
                var result = code switch
                {
                    ...
                    44 => WakeReason.Fingerprint,
                    0 => WakeReason.Unknown,
                    _ => WakeReason.Other
                };

                LogManager.LogInformation("[GoBackToSleep] Raw Reason code: {0}, mapped to: {1}", code, result);
                return result;
            }
            catch (Exception ex)
            {
                LogManager.LogWarning("[GoBackToSleep] ParseWakeReason exception: {0}", ex.Message);
                return WakeReason.Unknown;
            }
        }
    }
